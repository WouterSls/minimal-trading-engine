...
export class AerodromeStrategy implements ITradingStrategy {
   ...
   */
  constructor(private strategyName: string, private chain: ChainType) {
    ...
    this.FACTORY_ADDRESS = chainConfig.aerodrome.poolFactoryAddress;
  ...
  async ensureTokenApproval(
    ...
    if (TRADING_CONFIG.INFINITE_APPROVAL) {
      return await ensureInfiniteApproval(
        tokenAddress,
        amount,
        spender,
        wallet
      ...
      return await ensureStandardApproval(
        tokenAddress,
        amount,
        spender,
        wallet
    ...
    const routes: AerodromeTradeRoute[] = [
      {
        ...
        stable: false,
        factory: this.FACTORY_ADDRESS,
      },
    ];

    const inputAmount = ethers.parseUnits("1", this.WETH_DECIMALS);
    const amountOut = await this.router.getAmountsOut(
      wallet,
      inputAmount,
      routes
    );
  ...
  async getQuote(trade: TradeCreationDto, wallet: Wallet): Promise<Quote> {
    ...
    const inputToken: ERC20 | null = await createMinimalErc20(
      trade.inputToken,
      wallet.provider!
    );
    const outputToken: ERC20 | null = await createMinimalErc20(
      trade.outputToken,
      wallet.provider!
    );
    ...
    switch (tradeType) {
      case TradeType.ETHInputTOKENOutput:
        if (trade.inputType === InputType.USD) {
          ...
          const ethValue =
            parseFloat(trade.inputAmount) / parseFloat(ethUsdcPrice);
      ...
      case TradeType.TOKENInputTOKENOutput:
        if (!inputToken)
          throw new Error("Invalid input token for trade with token input");
        amountIn = ethers.parseUnits(
          trade.inputAmount,
          inputToken.getDecimals()
        );
        break;
      default:
    ...
    switch (tradeType) {
      ...
      case TradeType.TOKENInputTOKENOutput:
        if (!outputToken)
          throw new Error("Invalid output token for trade with token output");
    ...
    quote.route = await this.routeOptimizer.getBestAeroRoute(
      trade.inputToken,
      amountIn,
      trade.outputToken,
      wallet
    ...
    quote.outputAmount = ethers.formatUnits(
  ...
  async createTransaction(
    ...
    const inputToken = await createMinimalErc20(
      trade.inputToken,
      wallet.provider!
    );
    const outputToken = await createMinimalErc20(
      trade.outputToken,
      wallet.provider!
    );
    ...
    switch (tradeType) {
      case TradeType.ETHInputTOKENOutput:
        ...
        amountInForSpotRate = ethers.parseEther(
          TRADING_CONFIG.PRICE_IMPACT_AMOUNT_IN
        );
        break;
      case TradeType.TOKENInputETHOutput:
      case TradeType.TOKENInputTOKENOutput:
        ...
        amountInForSpotRate = ethers.parseUnits(
          TRADING_CONFIG.PRICE_IMPACT_AMOUNT_IN,
          inputToken.getDecimals()
        );
        break;
    ...
    const route = await this.routeOptimizer.getBestAeroRoute(
      trade.inputToken,
      amountIn,
      trade.outputToken,
      wallet
    ...
    if (!route.aeroRoutes)
      throw new Error("Error during Aerodrome route generation");

    const expectedOutput = await this.calculateExpectedOutput(
      amountInForSpotRate,
      amountIn,
      route.aeroRoutes,
      wallet
    ...
    switch (tradeType) {
      case TradeType.ETHInputTOKENOutput:
        tx = await this.router.createSwapExactETHForTokensTransaction(
          amountOutMin,
          route.aeroRoutes,
          to,
          deadline
        ...
        break;
      case TradeType.TOKENInputETHOutput:
        tx = this.router.createSwapExactTokensForETHTransaction(
          amountIn,
          amountOutMin,
          route.aeroRoutes,
          to,
